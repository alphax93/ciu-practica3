...
void writeBitmapString(float x, float y, void *font, const char *string) {
	const char *c;
	glRasterPos2f(x, y);
	for (c = string; *c != '\0'; c++) glutBitmapCharacter(font, *c);
}

void writeStrokeString(float x, float y, void *font, const char *string) {
	const char *c;
	glPushMatrix();
...
void Display() {
	...
	writeStrokeString(0.0, 0.0, GLUT_STROKE_ROMAN, "Hola OpenGL!");
